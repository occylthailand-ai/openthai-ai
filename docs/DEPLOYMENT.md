...
> `vercel.json` มี cron `/api/scheduler/process` แบบ **รายวัน** (Hobby รองรับเฉพาะรายวัน) เป็น fallback — **ความถี่ 15 นาทีจริงมาจาก UptimeRobot** ข้างบน (ใช้ได้ทุกแพลน) · ถ้าใช้ Vercel Pro ปรับ schedule เป็น `*/15 * * * *` ได้
