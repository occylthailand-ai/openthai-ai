    ...
    <div style={{ minHeight:'100vh', background:'#0a0a0f', color:'#fff', fontFamily:'system-ui,sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'16px 32px', borderBottom:'1px solid #1e1e2e' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          {['th','en','zh'].map(l => <button key={l} onClick={() => setLang(l)} style={{ background:lang===l?'#5e61f1':'none', border:'1px solid #333', color:'#fff', padding:'6px 12px', borderRadius:6, cursor:'pointer', fontSize:13 }}>{l==='th'?'ไทย':l==='en'?'English':'中文'}</button>)}
        </div>
      </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:32 }}>
          ...
          <div style={{ background:'#111', borderRadius:16, padding:28, border:'1px solid #6366f133' }}>
            ...
            <form onSubmit={submit}>
              ...
              <button type="submit" disabled={!consent} style={{ width:'100%', background:'#6366f1', color:'#fff', border:'none', padding:'14px', borderRadius:10, fontSize:16, fontWeight:700, cursor: consent ? 'pointer' : 'not-allowed', opacity: consent ? 1 : 0.5, marginTop:8 }}>{t.form.submit}</button>
            </form>}
          </div>
        </div>
      </div>
