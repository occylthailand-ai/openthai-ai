...
export default function GovIntlPortalPage() {
  ...
  const submit = async e => {
    e.preventDefault();
    setErr(''); setBusy(true);
    const r = await submitLead({ ...form, type:'gov-intl', lang, consent });
    setBusy(false);
    ...
    <div style={{ minHeight:'100vh', background:'#0a0a0f', color:'#fff', fontFamily:'system-ui,sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'16px 32px', borderBottom:'1px solid #1e1e2e' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          {['en','th','zh'].map(l => <button key={l} onClick={() => setLang(l)} type="button" aria-pressed={lang===l} style={{ background:lang===l?'#196cf4':'none', border:'1px solid #333', color:'#fff', padding:'6px 12px', borderRadius:6, cursor:'pointer', fontSize:13 }}>{l==='th'?'ไทย':l==='en'?'English':'中文'}</button>)}
        </div>
      </div>
          ...
          <div style={{ background:'#111', borderRadius:16, padding:28, border:'1px solid #3b82f633' }}>
            ...
            <form onSubmit={submit}>
              ...
              <label style={{ display:'flex', alignItems:'flex-start', gap:8, marginBottom:14, fontSize:12, color:'#aaa', lineHeight:1.5, cursor:'pointer' }}>
                ...
                <span>{consentLabel(lang, '#93c5fd')}</span>
              </label>
              ...
              <button type="submit" disabled={!consent || busy} style={{ width:'100%', background:'#3b82f6', color:'#fff', border:'none', padding:'14px', borderRadius:10, fontSize:16, fontWeight:700, cursor: (consent && !busy) ? 'pointer' : 'not-allowed', opacity: (consent && !busy) ? 1 : 0.5 }}>{busy ? '...' : t.form.submit}</button>
            </form>}
          </div>
        </div>
      </div>
