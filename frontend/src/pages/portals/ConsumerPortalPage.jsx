...
export default function ConsumerPortalPage() {
  ...
  const submit = async e => {
    e.preventDefault();
    setErr(''); setBusy(true);
    const r = await submitLead({ ...form, type:'consumer', lang, consent });
    setBusy(false);
    ...
    <div style={{ minHeight:'100vh', background:'#0a0a0f', color:'#fff', fontFamily:'system-ui,sans-serif' }}>
      <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', padding:'16px 32px', borderBottom:'1px solid #1e1e2e' }}>
        ...
        <div style={{ display:'flex', gap:8 }}>
          {['th','en','zh'].map(l => <button key={l} onClick={() => setLang(l)} type="button" aria-pressed={lang===l} style={{ background:lang===l?'#047f94':'none', border:'1px solid #333', color:'#fff', padding:'6px 12px', borderRadius:6, cursor:'pointer', fontSize:13 }}>{l==='th'?'ไทย':l==='en'?'English':'中文'}</button>)}
        </div>
      </div>
      <div style={{ maxWidth:900, margin:'0 auto', padding:'48px 32px' }}>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:32 }}>
          <div>
            <h3 style={{ color:'#06b6d4', marginBottom:20 }}>{benefitsTitle(lang)}</h3>
          ...
          <div style={{ background:'#111', borderRadius:16, padding:28, border:'1px solid #06b6d433' }}>
            {sent ? <div role="status" style={{ textAlign:'center', padding:32 }}><div style={{ fontSize:48 }}>✅</div><p style={{ color:'#10b981', marginTop:16 }}>{t.form.ok}</p><button type="button" onClick={() => navigate('/consumer/dashboard?email=' + encodeURIComponent(form.email))} style={{ marginTop:20, background:'#06b6d4', color:'#fff', border:'none', padding:'12px 22px', borderRadius:10, fontSize:15, fontWeight:700, cursor:'pointer' }}>{t.form.dashboard}</button></div> :
            <form onSubmit={submit}>
              ...
              <label style={{ display:'flex', alignItems:'flex-start', gap:8, marginBottom:16, fontSize:12, color:'#aaa', lineHeight:1.5, cursor:'pointer' }}>
                ...
                <span>{consentLabel(lang, '#67e8f9')}</span>
              </label>
              ...
              <button type="submit" disabled={!consent || busy} style={{ width:'100%', background:'#06b6d4', color:'#fff', border:'none', padding:'14px', borderRadius:10, fontSize:16, fontWeight:700, cursor: (consent && !busy) ? 'pointer' : 'not-allowed', opacity: (consent && !busy) ? 1 : 0.5, marginTop:8 }}>{busy ? '...' : t.form.submit}</button>
            </form>}
          </div>
        </div>
      </div>
