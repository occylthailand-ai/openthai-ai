...
export default function AffiliateDashboard() {
  ...
  const loadDashboard = async (code) => {
    ...
    try {
      const res = await fetchWithTimeout(apiUrl(`/api/affiliate/stats/${code.trim().toUpperCase()}`), {}, 10000);
          ...
          <div style={{ display: 'grid', gap: 20 }}>
            <div style={{ ...glass, border: '1.5px solid rgba(16,185,129,0.3)' }}>
              ...
              <div style={{ fontSize: 14, color: '#94a3b8', lineHeight: 1.8 }}>
                ...
                📩 แจ้งช่องทางรับเงินได้ที่ <a href="mailto:affiliate@openthai-ai.com" style={{ color: '#6366f1' }}>affiliate@openthai-ai.com</a>
              </div>
            </div>
          </div>
        )}
