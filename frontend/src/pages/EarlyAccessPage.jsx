...
export default function EarlyAccessPage() {
  ...
  const S = {
    page: { minHeight: '100vh', background: '#ffffff', color: '#0f172a', fontFamily: "'Inter','Sarabun',sans-serif" },
    nav: { background: 'rgba(255,255,255,0.95)', backdropFilter: 'blur(20px)', borderBottom: '1px solid #e2e8f0', padding: '14px 5%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', position: 'sticky', top: 0, zIndex: 100 },
    badge: { background: 'rgba(99,102,241,0.1)', border: '1px solid rgba(99,102,241,0.3)', borderRadius: 20, padding: '4px 14px', fontSize: 12, color: '#6366f1', fontWeight: 600 },
    section: { padding: '70px 5%', maxWidth: 960, margin: '0 auto' },
    sectionAlt: { padding: '70px 5%', background: '#f8fafc' },
    h2: { fontSize: 28, fontWeight: 800, marginBottom: 8, color: '#0f172a' },
    sub: { fontSize: 15, color: '#64748b', lineHeight: 1.7, marginBottom: 40 },
    card: { background: '#ffffff', border: '1px solid #e2e8f0', borderRadius: 14 },
    cardAlt: { background: '#f8fafc', border: '1px solid #e2e8f0', borderRadius: 14 },
    label: { display: 'block', fontSize: 13, color: '#475569', marginBottom: 6, fontWeight: 500 },
    input: { width: '100%', background: '#ffffff', border: '1px solid #cbd5e1', borderRadius: 8, padding: '10px 14px', color: '#0f172a', fontSize: 14, outline: 'none', boxSizing: 'border-box' },
    ...
    <div style={S.page}>
      ...
      <nav style={S.nav}>
        <a href="/" style={{ display: 'flex', alignItems: 'center', gap: 10, textDecoration: 'none' }}>
          <div style={{ width: 32, height: 32, background: 'linear-gradient(135deg,#6366f1,#10b981)', borderRadius: 8, display: 'flex', alignItems: 'center', justifyContent: 'center', fontWeight: 900, fontSize: 16, color: '#fff' }}>O</div>
          <span style={{ fontWeight: 800, fontSize: 16, color: '#0f172a' }}>OpenThai.ai</span>
        </a>
      ...
      <div style={{ padding: '80px 5% 50px', maxWidth: 960, margin: '0 auto', textAlign: 'center' }}>
        ...
        <p style={{ fontSize: 13, color: '#94a3b8' }}>ใช้ฟรีตลอดช่วง Early Access · ไม่ต้องใส่บัตรเครดิต</p>
      </div>

      {/* Proof strip */}
      <div style={{ borderTop: '1px solid #e2e8f0', borderBottom: '1px solid #e2e8f0', padding: '20px 5%', background: '#f8fafc' }}>
        <div style={{ maxWidth: 960, margin: '0 auto', display: 'flex', justifyContent: 'space-around', flexWrap: 'wrap', gap: 16 }}>
          {[['9', 'AI Programs พร้อมใช้'], ['3', 'ภาษา TH·ZH·EN'], ['12', 'ประเทศในระบบ Tax'], ['24/7', 'ระบบทำงาน']].map(([v, l]) => (
            <div key={l} style={{ textAlign: 'center' }}>
              <div style={{ fontSize: 26, fontWeight: 900, color: '#6366f1' }}>{v}</div>
            ...
            <div key={p.name} style={{ ...S.card, padding: '18px 20px', display: 'flex', gap: 14, alignItems: 'flex-start' }}>
              ...
              <div>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, background: 'rgba(16,185,129,0.12)', color: '#059669', borderRadius: 4, padding: '2px 6px', fontWeight: 600 }}>LIVE</span>
                </div>
          ...
          <div style={{ overflowX: 'auto', borderRadius: 14, border: '1px solid #e2e8f0', background: '#ffffff' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
              ...
              <tbody>
                {COMPARE_ROWS.map(r => (
                  <tr key={r.label}>
                    <td style={{ padding: '12px 16px', color: '#64748b', borderBottom: '1px solid #f1f5f9' }}>{r.label}</td>
                    <td style={{ padding: '12px 16px', textAlign: 'center', color: '#94a3b8', borderBottom: '1px solid #f1f5f9' }}>{r.them}</td>
                    <td style={{ padding: '12px 16px', textAlign: 'center', color: '#6366f1', fontWeight: 500, borderBottom: '1px solid #f1f5f9', background: 'rgba(99,102,241,0.03)' }}>{r.us}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill,minmax(260px,1fr))', gap: 16 }}>
          {PLANS.map(plan => (
            <div key={plan.name} style={{
              ...
              <div style={{ fontSize: 13, color: '#64748b', marginBottom: 6 }}>{plan.name}</div>
              <div style={{ display: 'flex', alignItems: 'baseline', gap: 4, marginBottom: 4 }}>
                <span style={{ fontSize: 32, fontWeight: 900, color: plan.color }}>{plan.price}</span>
                <span style={{ fontSize: 13, color: '#94a3b8' }}>{plan.sub}</span>
              </div>
      ...
      {/* Register Form */}
      <div id="register" style={{ background: '#f8fafc', padding: '70px 5%', borderTop: '1px solid #e2e8f0' }}>
        <div style={{ maxWidth: 520, margin: '0 auto' }}>
          ...
          {status === 'success' ? (
            <div style={{ ...S.card, padding: '40px 32px', textAlign: 'center' }}>
              ...
              <div style={{ fontSize: 20, fontWeight: 800, color: '#0f172a', marginBottom: 8 }}>ลงทะเบียนสำเร็จ!</div>
            ...
            <form onSubmit={handleSubmit} style={{ ...S.card, padding: '32px' }}>
              ...
              <button type="submit" disabled={status === 'loading'} style={{
                ...S.btn,
                background: status === 'loading' ? '#cbd5e1' : 'linear-gradient(90deg,#6366f1,#10b981)',
                color: '#fff',
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12, maxWidth: 680, margin: '32px auto 0' }}>
          {[
            ...
            <div key={q} style={{ ...S.card, padding: '16px 20px' }}>
              <div style={{ fontSize: 14, fontWeight: 600, color: '#0f172a', marginBottom: 6 }}>Q: {q}</div>
              <div style={{ fontSize: 13, color: '#64748b', lineHeight: 1.6 }}>A: {a}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {/* Footer CTA */}
      <div style={{ textAlign: 'center', padding: '60px 5%', borderTop: '1px solid #e2e8f0', background: '#f8fafc' }}>
        <h2 style={{ fontSize: 26, fontWeight: 800, marginBottom: 12, color: '#0f172a' }}>พร้อมเริ่มต้นแล้วใช่ไหม?</h2>
        <p style={{ fontSize: 15, color: '#64748b', marginBottom: 28 }}>ลงทะเบียนฟรี เริ่มใช้ 9 โปรแกรม AI ได้เลยทันที</p>
        ...
        <p style={{ fontSize: 12, color: '#94a3b8', marginTop: 16 }}>© 2026 OpenThai.ai · ไม่มีค่าใช้จ่ายซ่อนเร้น · ค่าตอบแทนโปร่งใส</p>
      </div>
