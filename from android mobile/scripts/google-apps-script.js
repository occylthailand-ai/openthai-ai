/**
 * OpenThaiAi × Claude — Google Apps Script
 * ==========================================
 * วิธีใช้:
...
function doGet(e) {
  ...
  return respond({ status: "OpenThaiAi — Google Apps Script is running ✅", ts: new Date().toISOString() });
}
...
function addWaitlist(data) {
  ...
  try {
    MailApp.sendEmail({
      to: ADMIN_EMAIL,
      subject: `🎉 OpenThaiAi — New Signup: ${data.name}`,
      body: `ลงทะเบียนใหม่!\n\nชื่อ: ${data.name}\nEmail: ${data.email}\nPhone: ${data.phone || "-"}\nRole: ${data.role || "-"}\nSource: ${data.source || "-"}\nPDPA: ${data.pdpa || "No"}\n\nเวลา: ${data.timestamp}`,
    });
